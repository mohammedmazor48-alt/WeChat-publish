...
class WeChatPublisher:
    ...
    def add_draft(self, title, content, thumb_url, author="深蓝", digest=""):
        """添加草稿到公众号"""
        if not self.access_token:
            print("✗ 未获取access_token")
            return False
...
def main():
    ...
    if not APPID or not APPSECRET:
        print("✗ 错误: 请在 .env 文件中配置 APPID 和 APPSECRET")
        return
    ...
    # 上传封面图
    print("\n[2/3] 上传封面图...")
    thumb_url = publisher.upload_thumb_media(str(cover_path))
    if not thumb_url:
        print("尝试使用本地路径继续...")
        thumb_url = ""
